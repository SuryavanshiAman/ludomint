...
class NetworkApiServices extends BaseApiServices {
  ...
  Future getGetApiResponse(String url) async {
    ...
    try {
      final response =
          await http.get(Uri.parse(url),).timeout( Duration(seconds: AppConstants.timeOut));
      if (kDebugMode) {
        print('Api Url : $url');
      }
  ...
  Future getPostApiResponse(String url, dynamic data) async {
    ...
    try {
      final response =
          await http.post(Uri.parse(url), body: data).timeout( Duration(seconds: AppConstants.timeOut));
